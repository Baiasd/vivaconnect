...
const ownerPage = ref<OwnerPage>() // 存储从API获取的数据
...
onMounted(() => {
  axios.get('/api/user/findOwnerPage').then(data => {
    ownerPage.value = data.data;
    console.log(ownerPage.value)
    console.log(data.data)
    if (ownerPage.value?.records) {
      ownerPage.value.records = ownerPage.value.records.map(owner => ({
        ...owner,
        phone: desensitizePhone(owner.phone),
        email: desensitizeEmail(owner.email)
      }))
    }
  })
  console.log(ownerPage.value)
...
const onDelete = (row: User) => {
  ...
  ElMessageBox.confirm(
    ...
    try {
      ...
      // 4. 刷新数据列表（网页4）
      const res = await axios.get('/api/user/findOwnerPage')
      ownerPage.value = res.data.records
    } catch (error) {
      // 5. 错误处理（网页6）
...
const handleCurrentChange = (val: number) => {
  ...
  axios.get('/api/user/findOwnerPage', {
    params: {page: val}
  }).then(data => {
    ownerPage.value = data.data;
    console.log(ownerPage.value)
...
<template>
  <el-row class="row-bg" justify="space-around" align="middle">
    ...
    <el-col :span="4">
      <div class="grid-content ep-bg-purple-light">
        ...
        <el-link disabled type="info">业主管理</el-link>
      </div>
    </el-col>
  </el-row>
  ...
  <el-card>
    <el-table
        ...
        :data="ownerPage?.records"
        row-key="oid"
        style="width: 100%"
        @selection-change="handleSelectionChange"
    >
      <el-table-column align="center" type="selection"/>
      <el-table-column align="center" property="username" label="业主名称"/>
    ...
    <div style="margin-top: 20px">
      ...
      <div class="example-pagination-block">
        <el-pagination layout="prev, pager, next" :total="ownerPage?.total" @size-change="handleSizeChange"
